...
1. A launchd job runs `bin/poll.js` on a fixed interval (`StartInterval` in
   ...
   collides with the next tick). launchd itself points at a small wrapper,
   `~/Library/Application Support/hermes-claude-bridge/run.sh` (generated by
   `install.sh`), not at the repo directly — the repo lives on an external
   disk that may be unmounted when a tick fires, and if `WorkingDirectory`
   or the log path don't resolve at spawn time, launchd fails the whole job
   with `EX_CONFIG` before any of our code runs. The wrapper checks the repo
   is reachable and exits 0 quietly if not; Telegram holds unfetched updates
   for ~24h, so a skipped tick only adds latency.
...
launchd/          plist + run.sh templates, filled in by install.sh
```
